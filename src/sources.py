...
from .habr_client import native_id as habr_native_id
...
_HABR_URL_RE = re.compile(r"career\.habr\.com/vacancies/(\d+)")


def parse_vacancy_url(url: str) -> tuple[str, str] | None:
    """Возвращает (source, native_id) по ссылке на вакансию hh.ru/superjob.ru/
    career.habr.com, либо None, если ссылка не распознана ни одним источником."""
    ...
    m = _HABR_URL_RE.search(url)
    if m:
        return "habr", m.group(1)
    return None


def get_full_vacancy(
    hh: HHClient, sj: SuperJobClient, vacancy_id: str, source: str | None, habr: HabrClient | None = None
) -> dict:
    if source == "superjob":
        return sj.get_vacancy(sj_native_id(vacancy_id))
    if source == "habr":
        if habr is None:
            raise RuntimeError("Источник habr, но HabrClient не передан в get_full_vacancy")
        return habr.get_vacancy(habr_native_id(vacancy_id))
...
def get_vacancy_status(
    ...
    if source == "habr":
        if habr is None:
            raise RuntimeError("Источник habr, но HabrClient не передан в get_vacancy_status")
        return habr.get_vacancy_status(habr_native_id(vacancy_id))
