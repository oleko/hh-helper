...
def _strip_tags(html: str, limit: int = 500) -> str:
    return re.sub(r"\s+", " ", _TAG_RE.sub(" ", html)).strip()[:limit]
...
def normalize(raw: dict) -> dict:
    ...
    # vacancyRichText — готовый HTML (SuperJob сам собирает "Обязанности"/"Требования"/
    # "Условия" туда), а work/candidat/compensation — их же расчленённые plain-text
    # копии, у многих вакансий вообще пустые (null). Раньше брали только их — вёрстка
    # терялась, описание выглядело обрубком голого текста. Теперь description — HTML
    # (та же форма, что и у HH: webapp._render_description прогоняет его через bleach
    # одинаково для обоих источников), snippet — короткий plain-text превью для
    # списка/фолбэка, из тех же полей или из richtext, если те пустые.
    plain_parts = [p for p in (raw.get("work"), raw.get("candidat"), raw.get("compensation")) if p]
    rich_text = raw.get("vacancyRichText")
    description = rich_text or "\n\n".join(plain_parts) or None
    snippet_text = " ".join(plain_parts) if plain_parts else (_strip_tags(rich_text) if rich_text else "")
    return {
        ...
        "snippet": {"requirement": snippet_text, "responsibility": None},
