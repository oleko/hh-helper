...
что делает браузер, когда у резюме на hh.ru выбран режим видимости «Видно по
прямой ссылке» (или более открытый). Приватные резюме так прочитать нельзя
(и не должно быть можно).
...
def fetch_resume_text(url: str, user_agent: str) -> str:
    ...
    if resp.status_code in (401, 403):
        raise ResumeFetchError(
            "резюме недоступно — скорее всего в настройках видимости на hh.ru "
            "не включён режим «Видно по прямой ссылке» (или более открытый)."
        )
    ...
    if len(text) < _MIN_TEXT_LENGTH:
        raise ResumeFetchError(
            ...
            "hh.ru изменил вёрстку, либо в настройках видимости резюме не "
            "включён режим «Видно по прямой ссылке» (страница выглядит как "
            "заглушка/логин, а не резюме)."
        )
    return text
